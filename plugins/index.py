...
async def index_files(bot, query):
    ...
    if lock.locked():
        return await query.answer('Wᴀɪᴛ ᴜɴᴛɪʟ ᴘʀᴇᴠɪᴏᴜꜱ ᴘʀᴏᴄᴇꜱs ᴄᴏᴍᴘʟᴇᴛᴇ.', show_alert=True)
    msg = query.message
...
async def send_for_index(bot, message):
    ...
    try:
        ...
        return await message.reply('Tʜɪꜱ ᴍᴀʏ ʙᴇ ᴀ ᴘʀɪᴠᴀᴛᴇ ᴄʜᴀɴɴᴇʟ / ɢʀᴏᴜᴘ. Mᴀᴋᴇ ᴍᴇ ᴀɴ ᴀᴅᴍɪɴ.')
    ...
    try:
        ...
        return await message.reply('ᴍᴀᴋᴇ sᴜʀᴇ ɪᴀᴍ ᴀᴅᴍɪɴ ɪɴ ᴛʜᴇ ᴄʜᴀɴɴᴇʟ')

    if getattr(k, "empty", False):
        return await message.reply('ɪᴀᴍ ɴᴏᴛ ᴀᴅᴍɪɴ ɪɴ ᴛʜɪs ɢʀᴏᴜᴘ')
    ...
    if type(chat_id) is int:
        try:
            ...
            return await message.reply('ᴀᴅᴍɪɴ ᴡɪᴛʜ ɪɴᴠɪᴛᴇ ʟɪɴᴋ ᴘᴇʀᴍɪssɪᴏɴ ʀᴇQᴜɪʀᴇᴅ.')
    else:
    ...
    await message.reply('Thanks! Wait for moderators to review.')
...
async def index_files_to_db(lst_msg_id, chat, msg, bot):
    ...
    async with lock:
        try:
            ...
            for message_id in range(lst_msg_id, 0, -1):
                if temp.CANCEL:
                    await msg.edit(
                ...
                try:
                    message = await bot.get_messages(chat, message_id)
                except Exception as e:
                    logger.warning(f"Failed msg {message_id}: {e}")
                    errors += 1
                    continue
                ...
                if current % 20 == 0:
                    can_btn = [[InlineKeyboardButton('Cancel', callback_data='index_cancel')]]
                    await msg.edit_text(
                        ...
                        f"(Unsupported - `{unsupported}`)\n"
                        f"Errors: <code>{errors}</code>",
                        reply_markup=InlineKeyboardMarkup(can_btn)
                    )
            ...
            await msg.edit(
                f'Successfully saved <code>{total_files}</code> files\n'
                f'Duplicate: <code>{duplicate}</code>\n'
                f'Deleted: <code>{deleted}</code>\n'
                f'Non-Media: <code>{no_media + unsupported}</code> '
                f"(Unsupported - `{unsupported}`)\n"
